...
int solve_system(double *matrix, double *result, size_t order)
{
    ...
    for(size_t i = 0; i < order; i++) {
        ...
        for(size_t j = i + 1; j < order; j++){
            s += SQUARE(matrix[COORDT(j, i, order)]);
        }

        norm1 = sqrt(SQUARE(matrix[COORDT(i, i, order)]) + s);
        ...
        matrix[COORDT(i, i, order)] -= norm1;
        norm2_square = SQUARE(matrix[COORDT(i, i, order)]) + s;
        ...
        for(size_t j = i + 1; j < order; j++) {
            ...
            for(size_t k = i; k < order; k++) {
                s += matrix[COORDT(k, i, order)] * matrix[COORD(k, j, order)];
            }
        ...
        s = 0.0;
        for(size_t k = i; k < order; k++) {
            s += matrix[COORDT(k, i, order)] * result[k];
        }

        s *= norm2_square;
        for(size_t k = i; k < order; k++) {
            result[k] -= s * matrix[COORDT(k, i, order)];
        }
    ...
    for(size_t i = 0/*order - 1*/; i < order/*>= 0*/; i++) {
        ...
        s = matrix[COORDT(order - 1 - i, order - 1 - i, order)];
        result[order - 1 - i] /= s;
        ...
        for(size_t k = 0; k < order - 1 - i; k++) {
            result[k] -= tmp * matrix[COORDT(k, order - 1 - i, order)];
        }
    }
